...
def rosinstall(pth, specfile):
    ...
    # to save testing time, we do not invoke rosinstall when we already have a .rosinstall file
    if not os.path.exists(os.path.join(pth, '.rosinstall')):
        succeed(["rosinstall", "-j8", "-n", pth, specfile, '--continue-on-error'], cwd=TESTS_DIR)
...
def create_catkin_workspace(pth):
    """
    prepares path to be a catkin workspace, by copying catkin and creating a CMakeLists.txt
    """
    ...
    assert(os.path.exists(pth + "/catkin/cmake/toplevel.cmake")), pth + "/catkin/cmake/toplevel.cmake"
...
class AbstractCatkinWorkspaceTest(unittest.TestCase):
    ...
    """
    Parent class for any test case that creates a workspace and calls cmake, make, and make install. Creates a suitable folder structure either in /tmp or in a root folder specified on init
    """
    ...
    def cmake(self,
        ...
        :param kwargs: (cwd, srcdir, expect) or stuff that will be added to the cmake command
        """
        args = []
        if cwd is None:
            cwd = self.builddir
