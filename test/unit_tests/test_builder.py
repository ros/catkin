...
class BuilderTest(unittest.TestCase):
    ...
    def test_build_workspace_in_isolation(self):
        ...
        try:
            ...
            build_workspace_in_isolation('/path1', root_dir)
            ...
            self.assertEqual(['cmake', '/foo', '-DCATKIN_STATIC_ENV=1'], run_args[0][0][0])
            self.assertEqual(['make', '-j8'], run_args[1][0][0])
            # install case
            mock_top.reset_mock()
            mock_run.reset_mock()
            build_workspace_in_isolation('/path1', root_dir, install=True)
            self.assertEqual(1, mock_top.call_count)
            top_args = mock_top.call_args[0]
            self.assertEqual(('/path1',), top_args)
            self.assertEqual(6, mock_run.call_count)
            run_args = mock_run.call_args_list
            self.assertEqual(['cmake', '/foo', '-DCATKIN_STATIC_ENV=1', '-DCMAKE_INSTALL_PREFIX=%s' % os.path.join(root_dir, 'build_isolated/p1__install')], run_args[0][0][0])
            self.assertEqual(['make', '-j8'], run_args[1][0][0])
